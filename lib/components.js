...
var EditComponent = (function (_super) {
  ...
  EditComponent.prototype.onCreated = function (service, param, showMessage, showError, confirm, getLocale, uis, loading, status, patchable, ignoreDate, backOnSaveSuccess) {
    var resourceService = input_1.getResource(param);
    this.resource = resourceService.resource();
    this.getLocale = input_1.getLocaleFunc(param, getLocale);
    this.loading = input_1.getLoadingFunc(param, loading);
    ...
    if (Array.isArray(x)) {
      ...
      if (x === st.success) {
        ...
        if (newMod && x === st.duplicate_key) {
          ...
          edit_1.handleStatus(x, st, this.resource, this.showError);
        }
      }
    }
    else {
    ...
    if (x && x !== '') {
      ...
      this.service.diff(_id).then(function (dobj) {
        ...
        if (data && data.status === 404) {
          ...
          core_1.error(err, com_3.resource, com_3.showError);
        }
        com_3.running = false;
        core_1.hideLoading(com_3.loading);
      });
